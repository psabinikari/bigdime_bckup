...
public class JDBCReaderHandler extends AbstractHandler {
		...
		if (!StringUtils.isEmpty(sql)) {
			...
			if (jdbcRuntimeManagement.findRTIEntryExistence(jdbcInputDescriptor.getEntityName(),
				...
				if (jdbcInputDescriptor.getIncrementedBy() != null
					...
					boolean runtimeInsertionFlag = jdbcRuntimeManagement.insertRuntimeEntry(jdbcInputDescriptor.getIncrementedColumnType(), 
															     driverName,jdbcInputDescriptor,initialRuntimeDateEntry, runTimeInfoStore);
					
					logger.info("JDBC Reader Handler inserting Runtime data","tableName={} PropertyKey={} PropertyValue={} status={}",
							jdbcInputDescriptor.getEntityName(),jdbcInputDescriptor.getIncrementedBy(),columnValue, runtimeInsertionFlag);
				}
				processFlag = processRecords();
			} else {
				logger.debug("JDBC Reader Handler processing an existing table ","tableName={}", jdbcInputDescriptor.getEntityName());
				processFlag = processRecords();
			}
	...
	public boolean processRecords() {
		...
		jdbcTemplate = new JdbcTemplate(sqlDataSource);
		String repoColumnValue = jdbcRuntimeManagement.findColumnValueByTableNameFromRTI(jdbcInputDescriptor,
			...
			if (columnValue != null) {
				try {
					if (driverName.indexOf(JdbcConstants.ORACLE_DRIVER) > JdbcConstants.INTEGER_CONSTANT_ZERO
						...
						rows = jdbcTemplate.queryForList(sql,new Object[] { new Timestamp(
		...
		if (jdbcInputDescriptor.getIncrementedBy().length() > JdbcConstants.INTEGER_CONSTANT_ZERO
			...
			boolean highestValueStatus = jdbcRuntimeManagement.saveCurrentHighestColumnValue(
							jdbcInputDescriptor.getIncrementedColumnType(),
	...
	/**
	 * This method sets each record in Action Event for further 
	 * Data cleansing
	 * @param rows
	 */
	private void processEachRecord(List<Map<String, Object>> rows) {
		...
		for (Map<String, Object> row : rows) {
			...
			System.out.println("byte count in JDBCReaderHandler" + body.length);
			actionEvents.add(actionEvent);
			
			highestIncrementalColumnValue = row.get(jdbcInputDescriptor.getColumnList().get(jdbcInputDescriptor.getColumnList().indexOf(
