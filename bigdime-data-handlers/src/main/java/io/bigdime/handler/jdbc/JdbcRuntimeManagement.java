...
public class JdbcRuntimeManagement {
	...
	private static final AdaptorLogger logger = new AdaptorLogger(
			LoggerFactory.getLogger(JdbcRuntimeManagement.class));
	 ...
	 * @return
	 */
	public int findRTIEntryExistence(String tableName,
			String incrementedColumnType,
			RuntimeInfoStore<RuntimeInfo> runTimeInfoStore) {
		// Find the existence in the Runtime...
		RuntimeInfo runtimeInfo = null;
		try {
			logger.debug(
					"JDBC Reader Handler checking runtime entry existence",
					"chekcing for TableName={} with column type={}", tableName,
					incrementedColumnType);
			if (incrementedColumnType != null
					&& incrementedColumnType.length() > JdbcConstants.INTEGER_CONSTANT_ZERO) {
				runtimeInfo = runTimeInfoStore.get("SQL_ADAPTOR", tableName,
						incrementedColumnType);
			}
			...
			logger.alert(
					...
					"\"runtimeInfo retrieval error\" inputDescription={} error={}",
					tableName, e.toString());
		}
		logger.debug(
				"JDBC Reader Handler after checking runtime entry existence",
				"Runtime info value = {}", runtimeInfo);
		if (runtimeInfo != null)
	 ...
	 * @return
	 */
	public boolean insertRuntimeEntry(String driverName,JdbcInputDescriptor jdbcInputDescriptor,
			String initialRuntimeDateEntry,RuntimeInfoStore<RuntimeInfo> runTimeInfoStore) {
		// Insert into RuntimeInfo
		boolean insertFlag = false;
		if (jdbcInputDescriptor.getIncrementedColumnType() != null) {
			RuntimeInfo runtimeInfo = new RuntimeInfo();
			runtimeInfo.setAdaptorName("SQL_ADAPTOR");
			runtimeInfo.setEntityName(jdbcInputDescriptor.getEntityName());
			runtimeInfo.setInputDescriptor(jdbcInputDescriptor
					.getIncrementedColumnType());
			...
			try {
				...
				logger.alert(
						...
						ALERT_CAUSE.APPLICATION_INTERNAL_ERROR,
						ALERT_SEVERITY.BLOCKER,
						"\"runtimeInfo insertion error\" inputDescription={} error={}",
						jdbcInputDescriptor.getEntityName(), e.toString());
			}
		}
		logger.debug(
				"JDBC Reader Handler inserting values in Runtime Information Managment(RTIM)",
				"inserting values for tableName={} with propertyKey={} . Insertion Staus (true/false)? ={}",
				jdbcInputDescriptor.getEntityName(),
				jdbcInputDescriptor.getIncrementedBy(), insertFlag);
	 ...
	 * @return
	 */
	public String findColumnValueByTableNameFromRTI(JdbcInputDescriptor jdbcInputDescriptor,
			RuntimeInfoStore<RuntimeInfo> runTimeInfoStore) {
		// Find the property value from Runtime...
		String repoColumnValue = null;
		RuntimeInfo runtimeInfo = null;
		logger.debug(
				"JDBC Reader Handler before getting current column entry value from runtime",
				"TableName={} inputDescriptor={}",
				jdbcInputDescriptor.getEntityName(),
				jdbcInputDescriptor.getIncrementedColumnType());
		try {
			if (jdbcInputDescriptor.getIncrementedColumnType() != null)
				runtimeInfo = runTimeInfoStore.get("SQL_ADAPTOR",
						jdbcInputDescriptor.getEntityName(),
						jdbcInputDescriptor.getIncrementedColumnType());
		} catch (RuntimeInfoStoreException e) {
		...
		logger.debug(
				"JDBC Reader Handler after getting current column entry value from runtime",
				"RuntimeInfo value = {}", runtimeInfo);

		if (runtimeInfo != null)
			repoColumnValue = runtimeInfo.getProperties().get(
	 ...
	 * @return
	 */
	public boolean saveCurrentHighestColumnValue(JdbcInputDescriptor jdbcInputDescriptor, String highestColumnValue,
			RuntimeInfoStore<RuntimeInfo> runTimeInfoStore) {
		...
		logger.debug(
				"JDBC Reader Handler saving values in Runtime Information Managment(RTIM)",
